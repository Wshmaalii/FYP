    ...
    <div className="flex-1 overflow-y-auto bg-zinc-950 px-8 py-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {spaces.map((space) => (
            <div key={space.conversation_key} className="rounded-[24px] border border-zinc-800 bg-zinc-900 p-6 shadow-[0_18px_40px_rgba(0,0,0,0.2)] transition-all duration-200 ease-out hover:border-zinc-700 hover:shadow-[0_24px_52px_rgba(0,0,0,0.24)]">
              <div className="flex items-start justify-between gap-5 mb-5">
                <div>
                  <div className="flex items-center gap-2 mb-2.5">
                    <h3 className="text-zinc-100 text-lg font-semibold tracking-tight">{space.name}</h3>
                    <span className="rounded border border-cyan-900/70 bg-cyan-950/50 px-2 py-1 text-[11px] uppercase tracking-wider text-cyan-300">
                      Public Space
                    </span>
                  </div>
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {space.channels.map((channel) => (
                  <span key={channel.channel_key} className="rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-xs text-zinc-400">
                    #{channel.slug}
                  </span>
                ))}
              </div>
